...
FOREACH (o IN CASE WHEN $newOrderVal < $originalOrderVal THEN [1] ELSE [] END | SET moveUpRestDownRel.order = toInteger(moveUpRestDownRel.order + 1) )
...
FOREACH (o IN CASE WHEN $newOrderVal > $originalOrderVal THEN [1] ELSE [] END | SET moveDownRestUpRel.order = toInteger(moveDownRestUpRel.order - 1) )

// Finally set the item that was moved to its position
SET orderRel.order = toInteger($newOrderVal)
`