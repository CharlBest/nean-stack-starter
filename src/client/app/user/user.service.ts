...
export class UserService {
    ...
    getUserPublic(userId: number): Promise<UserPublicViewModel | null> {
        return this.http.get<UserPublicViewModel>(UserRoutes.getUserPublic(userId).client()).toPromise();
    }

    getUserPublicItems(userId: number, tags: string | null, pageIndex: number, pageSize?: number): Promise<ItemViewModel[] | null> {
        return this.http.get<ItemViewModel[]>(UserRoutes.getUserPublicItems(userId).client({ tags, pageIndex, pageSize })).toPromise();
    }
